...
author: "Elliott O'Brien - edits by ryan"
date: "May 7, 2018"
...
test_hits <- Batting2017 %>%
  ...
  filter(
    ...
    Tm != 'TOT', 
    !is.na(H),
    playerID %in% active_batters_2016$playerID
    ) %>%  # at least one at bat during 2016 year, remove total rows
  select(playerID, H) %>%
  group_by(playerID) %>%
...
hits.lm <- active_batters_2016 %>%
  group_by(playerID) %>% 
  do(
    coefs = lm(H ~ yearID, data = .) %>% tidy %>% select(estimate) %>% unlist # testreplace with a function
  ) %>% 
  left_join( test_hits ) %>%
  mutate(
